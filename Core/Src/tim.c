...
void MX_TIM5_Init(void)
{
  ...
  /* USER CODE BEGIN TIM5_Init 2 */
	__HAL_TIM_CLEAR_IT(&htim5,TIM_IT_UPDATE);		//�����ж��ѿ����ͽ����ж�
	HAL_TIM_Base_Start_IT(&htim5);							//������ʱ��5�ж�
...
{
	encoder_count = TIM3->CNT;		//��ǰCNT��ֵ+�������*��ʱ�������崥����
	
//	Motor.speed_M_rpm = (float)(encoder_count - encoder_count_last) * 3.66211f;					//�����е���ٶ� speed = encoder_count_err/(����������*4)/(PWMZD_Count/TIM1_Frq)*60
	if(pi_spd.Ref >= 0)
	{
		if(encoder_count < encoder_count_last)
			Motor.speed_M_rpm = (float)(encoder_count + TIM3_Pulse - encoder_count_last)*Encoder_Fre/Encoder_Pulse;
		else
			Motor.speed_M_rpm = (float)(encoder_count - encoder_count_last)*Encoder_Fre/Encoder_Pulse;		
	}
	else
	{
		if(encoder_count > encoder_count_last)
			Motor.speed_M_rpm = (float)(encoder_count - (encoder_count_last + TIM3_Pulse))*Encoder_Fre/Encoder_Pulse;
		else
			Motor.speed_M_rpm = (float)(encoder_count - encoder_count_last)*Encoder_Fre/Encoder_Pulse;
	}
	
	Motor.speed_E_rpm = Motor.speed_M_rpm * Motor.P;																//����ٶ� = ��е���ٶ� * ������
...
{
	Motor.E_theta = (float)TIM3->CNT/Encoder_Pulse*PIX2*Motor.P;										//CNT/16384(������תһȦ����ֵ)*2PI*������
...
{
	//	if(htim == (&htim3))					//4096�߱�����
	//	{
	//		if(__HAL_TIM_IS_TIM_COUNTING_DOWN(&htim3))			//编码器计数
	//			encoder_overflow_count--;											//反转
	//		else
	//			encoder_overflow_count++;											//正转
	//	}
	if(htim == (&htim1))			//FOC����
	{
		BLDC_StepChange();
